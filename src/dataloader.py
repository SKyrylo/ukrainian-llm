...
# language-model training from pre-encoded token ID tensors.
#
...
#
# IMPORTANT: CustomDataset and create_dataloader now accept a pre-encoded
# 1-D token ID tensor rather than raw text + a tokenizer object.  Encoding
# must be done once by the caller (see main.py) so that the expensive BPE
# encode pass is not repeated for each dataset split.
import torch
from torch.utils.data import Dataset, DataLoader


# CustomDataset converts a 1-D tensor of pre-encoded token IDs into a
# collection of fixed-length overlapping windows suitable for batch training.
...
class CustomDataset(Dataset):
    # token_ids : 1-D integer tensor of pre-encoded token IDs for this split
    ...
    def __init__(self, token_ids: torch.Tensor, max_length: int, stride: int):
        self.input_ids = []
        self.target_ids = []
...
def create_dataloader(
    ...
    # Build the windowed dataset directly from the pre-encoded token IDs
...
if __name__ == "__main__":
    ...
    # Encode the text ONCE here before creating the DataLoader
    token_ids = tokenizer.encode(raw_text)
    # Create a small dataloader with context_length=4 for easy inspection
    dataloader = create_dataloader(token_ids, batch_size=8, context_length=4, stride=4, shuffle=False)
